	...
	if (_renderType != RenderType::None)
	{
		if (_renderType >= RenderType::D3D9 && _renderType <= RenderType::D3D12)
		{
			...
			else if (_renderType == RenderType::D3D12)
			{
				...
				// Get IDXGISwapChain3 for extended methods including Present1
				IDXGISwapChain3* swapChain3 = nullptr;
				swapChain->QueryInterface(__uuidof(IDXGISwapChain3), (void**)&swapChain3);

				// Expanded table: 150 base + 22 more for SwapChain1/2/3 (up to 40 swapchain methods total)
				...
				// Copy 40 methods from swapchain3 to include Present1 (index 22) and more
				if (swapChain3) {
